...
export const RecruiterDashboard = () => {
  ...
  const [filter, setFilter] = useState({ minScore: 0, jobId: '', experience: '', status: 'all' });
  ...
  const handleAddJob = async (e) => {
    e.preventDefault();
    const form = e.target;
    const newJob = { title: form.title.value, description: form.description.value };
  ...
  const handleToggleBookmark = async (candidate) => {
    ...
    setBookmarks(prev => {
      if (updatedCandidate.bookmarked) {
        ...
        return [...prev.filter(b => b.id !== updatedCandidate.id), updatedCandidate];
      } else {
        // Remove
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-teal-500 to-indigo-600'} text-gray-100`}>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <Header user={user} onLogout={handleLogout} darkMode={darkMode} />
        <TabNavigation activeTab={activeTab} setActiveTab={setActiveTab} darkMode={darkMode} />

        {activeTab === 'candidates' && (
          <>
            <FilterPanel filter={filter} setFilter={setFilter} jobDescriptions={jobs} darkMode={darkMode} />
            <div>
              {filtered.map(c => (
                <CandidateCard
                  key={c.id}
        ...
        {activeTab === 'jds' && (
          <>
            <h2 className="text-2xl font-semibold mb-4 text-white">Job Descriptions</h2>
            <form onSubmit={handleAddJob} className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <input name="title" placeholder="Title" className="px-3 py-2 rounded-lg border bg-white/80 text-black" required />
              <input name="description" placeholder="Description" className="px-3 py-2 rounded-lg border bg-white/80 text-black" required />
              <button type="submit" className="px-4 py-2 rounded-lg bg-blue-500 text-white font-semibold">Add JD</button>
            </form>
            <div>
              {jobs.map(job => (
                <JobCard key={job.id} job={job} onDelete={handleDeleteJob} darkMode={darkMode} />
              ))}
            </div>
          </>
        )}
        ...
        {activeTab === 'bookmarks' && (
          <>
            <h2 className="text-2xl font-semibold mb-4 text-white">Bookmarks</h2>
            <div>
              {bookmarks.map(b => (
                <BookmarkCard
                key={b.id}
